...
LastEditTime: 2022-11-27 16:07:23
Description: 
'''
# coding:utf-8
import os
...
def get_dataset(file_pattern, batch_size=1):
    ...
    return (
        ...
        tf.data.TFRecordDataset(file_pattern)
        .map(
            read_example,
...
class CLIP(tf.keras.Model):
    ...
    def call(self, features, training=False):
        ...
        image_embeddings = vision_encoder(features["image"])
...
batch_size = 64
bert_config_file = "/data3/heboyu/pretrain_model/distilbert_tf/bert_config.json"
...
text_encoder = create_text_encoder(
    max_seq_length=256, bert_config_file=bert_config_file, is_training=True, final_hidden_size=128
)
#text_encoder.load_weights("./model_2l_6-10e_128b_768d/model.ckpt-4").expect_partial()
text_encoder.load_weights("./model_2l_10e_128b_128d/model.ckpt-4").expect_partial()
...
tfrecord_file_list = []
tf_record_dir = ["/data3/hhz_image_caption_tmp", "/data2/zhangyuanhang/nlp_task/multimodal_embedding/data"]
for dir in tf_record_dir:
    for i in os.listdir(dir):
        if not (i.startswith("clip_train_data") and i.endswith(".tfrecord")):
            continue
        tfrecord_file_list.append(os.path.join(dir, i)) 
        print(os.path.join(dir, i))

train_dataset = get_dataset(tfrecord_file_list, batch_size)
valid_dataset = get_dataset(tfrecord_file_list[:1], batch_size)


# Create a learning rate scheduler callback.
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(
    monitor="loss", factor=0.2, patience=5
)
