...
def main(TrainParams):
    ...
    '''
    text_encoder.load_weights("model_2l_10e_128b/text_encoder_model.h5", by_name=True)
    #text_encoder.load_weights(TrainParams.init_checkpoint, by_name=True)
    ...
    if TrainParams.is_training:
        ...
        # load model
        #text_encoder.load_weights("./model_2l_ss_10e/latest_model.weights")
        ...
        ####### test predict
        inputs, masks = [], []
        for text in ["厨房", "卫生间", "燃气灶", "马桶", "厕所", "客厅", "沙发"]:
        #for text in ["简约美式厨房", "简约风的客厅看着赏心悦目", "卧室也是属于简约风", "卧室里有一张白色的大床","客厅里有一组黑色的皮质沙发", "120平的大三居"]:
        #for text in ["简约美式厨房"]:
            input_ids, input_mask, _ = ConvertTextToIds()(text)
            inputs.append(input_ids)
            masks.append(input_mask)
        ret = text_encoder([np.array(inputs), np.array(masks)])
        score = cosine_similarity(ret, ret)
        print(score, ret.shape)
...
class TrainParams():
    ...
    init_checkpoint = "/data3/heboyu/pretrain_model/distilbert_tf/bert_model.ckpt"
